							...
							if (!ChanDiscordBot.ChanData.DataStore.PostIdsPosted.Contains(post.no))
							{
								foreach (var server in ChanDiscordBot.BotConfig.BotInfo.ServersToSend.Where(x => x.Boards.Any(y => y.ToUpper().Contains(b.BoardName.ToUpper()))))
								{
									...
									foreach (var channelid in server.ChannelIds)
									{
										...
										eb.Title = $"{(!string.IsNullOrWhiteSpace(post.sub) ? $"{post.sub} - " : "")}{(!string.IsNullOrWhiteSpace(post.name) ? $"{post.name} - " : "")}No. {post.no}";
										...
										{
											eb.AddField("Image", $"{post.filename}{post.ext} - {post.w}x{post.h} ({(float)post.fsize / 1024 / 1024:0.##}MiB)");
